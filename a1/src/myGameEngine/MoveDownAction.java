...
public class MoveDownAction extends AbstractInputAction {
	private Camera cam;
	private MyGame mygame;
	
	public MoveDownAction(Camera c, MyGame g) {
		cam    = c;
		mygame = g;
	}
	@Override
	public void performAction(float time, Event e) {
		if(mygame.camera.getMode() == 'c') {      // if the view mode is set to 'c' (OFF DOLPHIN)
			System.out.println("Moving Down || MODE: 'C'");
			Vector3f v   =   cam.getUp();
			...
			System.out.println("Moving Down || MODE: 'N'");
			mygame.dolphinN.moveUp(-0.15f);
		}
