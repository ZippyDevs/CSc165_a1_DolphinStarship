...
public class MoveYAction_C extends AbstractInputAction {
	private Camera cam;
	private MyGame mygame;
	
	public MoveYAction_C(Camera c, MyGame g) {
		cam    = c;
		mygame = g;
	}
	@Override
	public void performAction(float time, Event e) {
		if(e.getValue() <= -0.2f) {
			...
			if(mygame.camera.getMode() == 'c') { // if the view mode is set to 'c' (OFF DOLPHIN)
				System.out.println("Moving Backward || MODE: 'C'");
				Vector3f v   =   cam.getFd();
				Vector3f p   =   cam.getPo();
				Vector3f p1  =   (Vector3f)Vector3f.createFrom(-0.15f*v.x(),-0.15f*v.y(), -0.15f*v.z());
				Vector3f p2  =   (Vector3f)p.add((Vector3)p1);
				...
				System.out.println("Moving Backward || MODE: 'N'");
